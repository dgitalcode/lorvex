...
export default async function CollectionPage({ params }: Props) {
  ...
  return <div className="pb-24 page-pad">
    ...
    <header className="relative flex min-h-[52vh] items-end overflow-hidden bg-[#171512]">{collection.coverUrl && <Image src={collection.coverUrl} alt={collection.name} fill priority className="object-cover opacity-65" />}<div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" /><div className="luxury-container relative pb-14 text-white"><p className="text-[11px] uppercase tracking-[.24em] text-[#d6bd8b]">{collection.isLimited ? copy.collectionLimitedEyebrow : copy.collectionEyebrow}</p><h1 className="mt-3 font-display text-6xl md:text-7xl">{collection.name}</h1>{collection.description && <p className="mt-4 max-w-2xl text-white/75">{collection.description}</p>}</div></header>
    <div className="luxury-container mt-14"><p className="mb-8 text-sm text-muted-foreground">{copy.collectionPieces(result.total)}</p><div className="grid grid-cols-2 gap-5 lg:grid-cols-4">{result.products.map((product) => <ProductCard key={product.id} product={product} locale={locale} />)}</div></div>
  </div>;
}
